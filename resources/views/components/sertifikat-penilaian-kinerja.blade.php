...
<section class="py-20 bg-orange-500 overflow-hidden">
    <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-12">
            <h2 class="text-3xl md:text-4xl font-bold text-white mb-4">SERTIFIKAT PENILAIAN KINERJA</h2>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($items as $index => $item)
                ...
                @endphp
                <button type="button" onclick="openCertificateModal('{{ $imageUrl ?? '' }}', '{{ addslashes($item->judul) }}')" class="group text-left bg-white/10 backdrop-blur-sm rounded-2xl overflow-hidden border border-white/20 shadow-lg hover:-translate-y-1 transition duration-300">
                    @if($imageUrl)
                    ...
                    @endif
                    <div class="p-5">
                        <h3 class="text-lg font-semibold text-white uppercase">{{ $item->judul }}</h3>
    ...
    <div id="certificateModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/80 px-4 py-6">
        <div class="relative w-full max-w-4xl rounded-3xl bg-white p-3 shadow-2xl">
            <button type="button" onclick="closeCertificateModal()" class="absolute right-3 top-3 z-10 rounded-full bg-slate-900/80 p-2 text-white hover:bg-slate-700">
