...
<div x-show="activeTab === 'legalitas'" class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-200 dark:border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-blue-400"></span> Pengaturan Halaman: Legalitas Perusahaan
        </h2>
        <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Kelola data legalitas, NIB, Izin Usaha Jasa Ketenagalistrikan (IUJK), Sertifikat Akreditasi ESDM, dan lampiran berkas PDF resmi.</p>
    </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'LEGALITAS PERUSAHAAN') }}" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="5" placeholder="Contoh:&#10;• NIB: 1234567890&#10;• IUJK ESDM: No. 503/IUJK/2024&#10;• Akreditasi LIT: Kementerian ESDM RI" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">{{ old('konten', $data->konten ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-3 bg-slate-50/80 dark:bg-slate-950/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                    <label class="block text-xs font-bold text-blue-400 uppercase tracking-wider">Upload Lampiran PDF Sertifikat / Dokumen Legalitas Resmi</label>
                    @if(optional($data)->url_dokumen)
                        <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl">
                            <div class="flex items-center gap-2 overflow-hidden">
                                ...
                                <span class="text-xs text-slate-700 dark:text-slate-300 truncate">Dokumen Legalitas PDF Ter-upload</span>
                            </div>
