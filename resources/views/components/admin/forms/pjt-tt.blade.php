...
<div x-show="activeTab === 'daftar-pjt'" class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-200 dark:border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-blue-400"></span> Pengaturan Halaman: Daftar PJT & TT
        </h2>
        <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Kelola judul header dan dokumen PDF resmi daftar Penanggung Jawab Teknik (PJT) & Tenaga Teknik (TT) terdaftar Kementrian ESDM.</p>
    </div>

    <form action="{{ route('admin.profil.halaman.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <input type="hidden" name="halaman" value="profil_pjt_tt">
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'DAFTAR PJT & TT') }}" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="3" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">{{ old('konten', $data->konten ?? 'Seluruh Tenaga Teknik PT Pradana Nusa Energi telah memiliki Sertifikat Kompetensi (Serkom) yang diterbitkan oleh Lembaga Sertifikasi Kompetensi (LSK) terakreditasi Kementerian ESDM.') }}</textarea>
                </div>
            </div>

            <div class="space-y-6">
                <!-- Upload Dokumen PDF Resmi PJT TT -->
        ...
        <div class="flex justify-end pt-2 border-t border-slate-200 dark:border-slate-800">
            <button type="submit" class="bg-gradient-to-r from-blue-600 to-sky-500 hover:from-blue-700 hover:to-sky-600 text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-blue-600/20 transition flex items-center gap-2">
                ...
                Simpan Perubahan Halaman PJT & TT
            </button>
        </div>
    </form>
</div>
