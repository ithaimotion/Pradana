...
<div x-show="activeTab === 'daftar-pjt'" class="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-amber-400"></span> Pengaturan Halaman: Daftar PJT & TT
        </h2>
        <p class="text-xs text-slate-400 mt-1">Kelola judul header dan dokumen PDF resmi daftar Penanggung Jawab Teknik (PJT) & Tenaga Teknik (TT) terdaftar Kementrian ESDM.</p>
    </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'DAFTAR PJT & TT') }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1 uppercase tracking-wider">Deskripsi Sub-Header</label>
                    <textarea name="subjudul" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('subjudul', $data->subjudul ?? 'Daftar Penanggung Jawab Teknik (PJT) dan Tenaga Teknik (TT) terdaftar dan bersertifikasi kompetensi resmi PT Pradana Nusa Energi.') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('konten', $data->konten ?? 'Seluruh Tenaga Teknik PT Pradana Nusa Energi telah memiliki Sertifikat Kompetensi (Serkom) yang diterbitkan oleh Lembaga Sertifikasi Kompetensi (LSK) terakreditasi Kementerian ESDM.') }}</textarea>
                </div>
            </div>
