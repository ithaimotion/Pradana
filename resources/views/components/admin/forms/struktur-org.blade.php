...
<div x-show="activeTab === 'struktur-org'" class="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-amber-400"></span> Pengaturan Halaman: Struktur Organisasi
        </h2>
        <p class="text-xs text-slate-400 mt-1">Upload foto diagram bagan Struktur Organisasi PT Pradana Nusa Energi dan kelola penjelasan jajaran manajemen.</p>
    </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'STRUKTUR ORGANISASI') }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1 uppercase tracking-wider">Sub-Judul / Tagline</label>
                    <textarea name="subjudul" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('subjudul', $data->subjudul ?? 'Bagan Struktur Organisasi dan Manajemen PT Pradana Nusa Energi.') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="5" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('konten', $data->konten ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="border-2 border-dashed border-slate-800 rounded-xl p-5 text-center hover:border-amber-500/50 transition">
