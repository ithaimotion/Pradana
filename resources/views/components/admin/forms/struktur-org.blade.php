...
<div x-show="activeTab === 'struktur-org'" class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-200 dark:border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-blue-400"></span> Pengaturan Halaman: Struktur Organisasi
        </h2>
        <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Upload foto diagram bagan Struktur Organisasi PT Pradana Nusa Energi dan kelola penjelasan jajaran manajemen.</p>
    </div>

    <form action="{{ route('admin.profil.halaman.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <input type="hidden" name="halaman" value="profil_struktur">
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'STRUKTUR ORGANISASI') }}" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <textarea name="konten" rows="5" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">{{ old('konten', $data->konten ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                @if(optional($data)->url_gambar)
                    <div class="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 h-52 bg-slate-50 dark:bg-slate-950 relative">
                ...
                <div class="border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-5 text-center hover:border-blue-500/50 transition">
        ...
        <div class="flex justify-end pt-2 border-t border-slate-200 dark:border-slate-800">
            <button type="submit" class="bg-gradient-to-r from-blue-600 to-sky-500 hover:from-blue-700 hover:to-sky-600 text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-blue-600/20 transition flex items-center gap-2">
                ...
                Simpan Bagan Struktur Organisasi
            </button>
        </div>
    </form>
</div>
