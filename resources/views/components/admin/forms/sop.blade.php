...
<div x-show="activeTab === 'sop'" class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-200 dark:border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-blue-400"></span> Pengaturan Halaman: Standar Operasi Prosedur (SOP)
        </h2>
        <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Kelola pedoman SOP inspeksi teknik kelistrikan, sertifikasi K3, dan upload berkas PDF panduan operasional resmi.</p>
    </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'STANDAR OPERASI PROSEDUR (SOP)') }}" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 uppercase tracking-wider">Sub-Judul / Keterangan SOP</label>
                    <textarea name="subjudul" rows="2" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">{{ old('subjudul', $data->subjudul ?? 'Seluruh tahapan inspeksi ketenagalistrikan dilaksanakan secara ketat berlandaskan SOP dan K3.') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 uppercase tracking-wider">Daftar Kode & Prosedur SOP Utama</label>
                    <textarea name="konten" rows="5" placeholder="Contoh:&#10;• SOP-INSP-01: Prosedur Keselamatan Kerja K3 Inspeksi&#10;• SOP-INSP-02: Pemeriksaan & Pengujian Instalasi TR&#10;• SOP-INSP-03: Pemeriksaan & Pengujian Instalasi TM" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">{{ old('konten', $data->konten ?? '') }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-3 bg-slate-50/80 dark:bg-slate-950/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                    <label class="block text-xs font-bold text-blue-400 uppercase tracking-wider">Upload Berkas PDF Dokumen Manual SOP Resmi</label>
                    @if(optional($data)->url_dokumen)
                        <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl">
                            <div class="flex items-center gap-2 overflow-hidden">
                                ...
                                <span class="text-xs text-slate-700 dark:text-slate-300 truncate">Dokumen SOP PDF Ter-upload</span>
                            </div>
