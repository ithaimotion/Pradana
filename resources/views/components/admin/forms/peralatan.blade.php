...
<div x-show="activeTab === 'peralatan'" class="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl" x-cloak>
    <div class="border-b border-slate-800 pb-4">
        <h2 class="text-xl font-bold text-white flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-amber-400"></span> Pengaturan Halaman: Peralatan Ketenagalistrikan
        </h2>
        <p class="text-xs text-slate-400 mt-1">Kelola data alat ukur, instrumen uji komisioning, status kalibrasi berkala, dan dokumentasi alat kelistrikan.</p>
    </div>
        ...
        <div class="grid md:grid-cols-2 gap-8">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul', $data->judul ?? 'PERALATAN UJI KETENAGALISTRIKAN') }}" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1 uppercase tracking-wider">Deskripsi Sub-Header</label>
                    <textarea name="subjudul" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('subjudul', $data->subjudul ?? 'Seluruh peralatan uji instrumen PT Pradana Nusa Energi telah terkalibrasi secara presisi dan berkala.') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1 uppercase tracking-wider">Daftar Peralatan & Spesifikasi Utama</label>
                    <textarea name="konten" rows="5" placeholder="Contoh:&#10;• Insulation Resistance Tester (Megger 5kV/10kV)&#10;• Secondary Current Injection Test Set&#10;• Earth Tester / Grounding Resistance Meter&#10;• Thermal Imaging Camera / Thermography Inspection" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">{{ old('konten', $data->konten ?? '') }}</textarea>
                </div>
            </div>
