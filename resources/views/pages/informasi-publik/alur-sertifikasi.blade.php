            ...
            @if($alurSertifikasi && $alurSertifikasi->is_active && $alurSertifikasi->path_pdf)
                <div class="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden reveal-on-scroll">
                    ...
                    <div class="bg-gradient-to-r from-blue-900 to-blue-800 px-6 py-4 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            <button onclick="openFullscreen()" class="flex items-center gap-2 bg-slate-100 dark:bg-white/10 hover:bg-white/20 text-slate-900 dark:text-white px-4 py-2 rounded-lg transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4"></path>
                                </svg>
                                <span class="hidden sm:inline">Full Screen</span>
                            </button>
                            <a href="{{ asset('storage_public/' . $alurSertifikasi->path_pdf) }}" download class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="relative" style="height: 700px;">
                        <iframe 
                            src="{{ asset('storage_public/' . $alurSertifikasi->path_pdf) }}" 
                            class="w-full h-full"
                            frameborder="0"
                            allowfullscreen>
                        </iframe>
