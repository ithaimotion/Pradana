                ...
                @foreach($itemsByLevel as $level => $items)
                    @if($level == 1)
                        ...
                        <div class="flex justify-center mb-0 reveal-on-scroll">
                            <div class="relative">
                                <div class="bg-gradient-to-br from-blue-900 to-slate-900 text-white rounded-2xl shadow-2xl px-10 py-7 text-center min-w-[280px] border border-slate-200 dark:border-white/10">
                                    ...
                                    <div class="text-xs font-semibold uppercase tracking-widest text-blue-500 mb-1">{{ $items[0]->jabatan ?? 'Top Management' }}</div>
                                    <div class="text-xl font-extrabold">{{ $items[0]->nama }}</div>
                                    @if($items[0]->divisi)
                                        <div class="text-xs text-slate-700 dark:text-slate-300 mt-1">{{ $items[0]->divisi }}</div>
                                    @endif
                                </div>
                            ...
                            @foreach($items as $index => $item)
                                <div class="relative">
                                    <div class="bg-white border-2 border-blue-900/20 rounded-2xl shadow-lg px-8 py-6 text-center min-w-[240px] hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
                                        <div class="w-12 h-12 {{ $index % 2 == 0 ? 'bg-blue-900' : 'bg-blue-600' }} rounded-xl flex items-center justify-center mx-auto mb-3 shadow-md">
                                        ...
                                        <div class="text-xs font-semibold uppercase tracking-widest {{ $index % 2 == 0 ? 'text-blue-900' : 'text-blue-600' }} mb-1">{{ $item->jabatan }}</div>
                    ...
                    @else
                        ...
                        <div class="mt-14 reveal-on-scroll delay-400">
                            <div class="text-center mb-8">
                                <span class="text-xs uppercase tracking-widest font-semibold text-slate-600 dark:text-slate-400">Level {{ $level }} Staff</span>
                            </div>
                                ...
                                @foreach($items as $item)
                                    <div class="bg-slate-100 border border-slate-200 rounded-xl px-5 py-4 flex items-center gap-4 hover:shadow-md hover:-translate-y-0.5 transition-all">
                                        ...
                                        <div>
                                            ...
                                            @if($item->divisi)
                                                <div class="text-xs text-slate-600 dark:text-slate-400">{{ $item->divisi }}</div>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
