...
<div class="space-y-6">
    <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div class="mb-6">
            ...
            <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Edit daftar Penanggung Jawab Teknik (PJT) dan Tenaga Teknik (TT)</p>
        </div>
        ...
        <form action="{{ route('admin.profil.daftar-pjttt.update', $daftarPJTTT->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul', $daftarPJTTT->judul) }}" placeholder="DAFTAR PJT & TT" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                        @error('judul')
                    ...
                    <div>
                        ...
                        <textarea name="subjudul" rows="3" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500" placeholder="Daftar Penanggung Jawab Teknik (PJT) dan Tenaga Teknik (TT)...">{{ old('subjudul', $daftarPJTTT->subjudul) }}</textarea>
                        @error('subjudul')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 uppercase tracking-wider">Catatan Kualifikasi / Akreditasi ESDM</label>
                        <textarea name="konten" rows="3" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500" placeholder="Seluruh Tenaga Teknik PT Pradana Nusa Energi telah memiliki Sertifikat Kompetensi...">{{ old('konten', $daftarPJTTT->konten) }}</textarea>
                        @error('konten')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <div class="space-y-3 bg-slate-50/80 dark:bg-slate-950/50 p-5 rounded-xl border border-slate-200 dark:border-slate-800">
                        <label class="block text-xs font-bold text-blue-400 uppercase tracking-wider">Dokumen PDF Resmi SK PJT & TT</label>
            ...
            <div class="flex items-center gap-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                <a href="{{ route('admin.profil.daftar-pjttt.index') }}" class="px-6 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white text-sm font-medium transition">
                    Batal
                </a>
    ...
    <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div class="mb-6">
            ...
            <p class="text-xs text-slate-600 dark:text-slate-400 mt-1">Tambah, edit, atau hapus item Penanggung Jawab Teknik (PJT) dan Tenaga Teknik (TT)</p>
        </div>

        <!-- Add New Item Form -->
        <div class="bg-slate-50/60 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl p-6 mb-6">
            ...
            <form action="{{ route('admin.profil.daftar-pjttt.items.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="kategori" placeholder="Instalasi Pemanfaatan Tenaga Listrik Tegangan Menengah" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                        @error('kategori')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="jabatan" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="no_sertifikat" placeholder="SKT-XXX-XXX" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                        @error('no_sertifikat')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">No. Register</label>
                        <input type="text" name="no_register" placeholder="REG-XXX-XXX" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                        @error('no_register')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <!-- Existing Items List -->
        <div class="space-y-4">
            <h3 class="text-sm font-bold text-blue-400 uppercase tracking-wider">Daftar Item ({{ $daftarPJTTT->items->count() }})</h3>
            @if($daftarPJTTT->items->count() > 0)
                <div class="space-y-3">
                    @foreach($daftarPJTTT->items as $item)
                        <div class="bg-slate-50/60 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-xl p-4" id="item-{{ $item->id }}">
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center gap-3">
                                    <span class="px-2 py-1 rounded text-xs font-bold {{ $item->jabatan === 'PJT' ? 'bg-blue-900 text-blue-200' : 'bg-blue-600 text-white' }}">
                                        {{ $item->jabatan }}
                                    </span>
                                    <span class="text-sm font-medium text-slate-900 dark:text-white">{{ $item->nama }}</span>
                                </div>
                            ...
                            <div class="grid md:grid-cols-3 gap-4 text-xs">
                                <div>
                                    <span class="text-slate-600 dark:text-slate-400">Kategori:</span>
                                    <span class="text-slate-800 dark:text-slate-200 ml-1">{{ $item->kategori }}</span>
                                </div>
                                <div>
                                    <span class="text-slate-600 dark:text-slate-400">No. Sertifikat:</span>
                                    <span class="text-slate-800 dark:text-slate-200 ml-1">{{ $item->no_sertifikat }}</span>
                                </div>
                                <div>
                                    <span class="text-slate-600 dark:text-slate-400">No. Register:</span>
                                    <span class="text-slate-800 dark:text-slate-200 ml-1">{{ $item->no_register }}</span>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
<div id="editModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50">
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 w-full max-w-lg mx-4 shadow-2xl">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">Edit Item PJT & TT</h3>
        ...
        <form id="editItemForm" action="" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Jabatan</label>
                <select name="jabatan" id="editJabatan" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
                    <option value="PJT">PJT (Penanggung Jawab Teknik)</option>
                    <option value="TT">TT (Tenaga Teknik)</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">No. Sertifikat</label>
                <input type="text" name="no_sertifikat" id="editNoSertifikat" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">No. Register</label>
                <input type="text" name="no_register" id="editNoRegister" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-blue-500">
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Urutan</label>
