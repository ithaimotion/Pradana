...
<div class="space-y-6">
    <div class="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
        <div class="mb-6">
            ...
            <p class="text-xs text-slate-400 mt-1">Isi form di bawah untuk membuat legalitas perusahaan baru</p>
        </div>
            ...
            <div class="grid md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}" placeholder="LEGALITAS PERUSAHAAN" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500">
                        @error('judul')
                    ...
                    <div>
                        ...
                        <textarea name="subjudul" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-sm text-slate-100 focus:outline-none focus:border-amber-500" placeholder="PT Pradana Nusa Energi beroperasi secara legal dan resmi berlandaskan izin Kementerian ESDM & Pemerintah RI.">{{ old('subjudul') }}</textarea>
                        @error('subjudul')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
