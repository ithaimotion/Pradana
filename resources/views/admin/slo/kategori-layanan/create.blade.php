...
@section('title', 'Tambah Kategori Layanan SLO')

@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center gap-4">
        <a href="{{ route('admin.slo.kategori-layanan.index') }}" class="p-2 text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-white">Tambah Kategori Layanan SLO</h1>
            <p class="text-slate-400 text-sm mt-1">Tambah kategori layanan inspeksi baru</p>
        </div>
    </div>

    <!-- Form -->
    <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
        <form action="{{ route('admin.slo.kategori-layanan.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Kategori Utama -->
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Kategori Utama <span class="text-rose-400">*</span></label>
                <select name="kategori_utama" required
                    ...
                    @foreach($kategoriOptions as $value => $label)
                        <option value="{{ $value }}" {{ old('kategori_utama') == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <!-- Judul -->
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Judul Kategori <span class="text-rose-400">*</span></label>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" required
                    ...
                    placeholder="Deskripsi lengkap tentang kategori layanan">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-rose-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- Tags -->
            <div>
                <label class="block text-sm font-semibold text-slate-300 mb-2">Tags (Opsional)</label>
                <input type="text" name="tags[]" value="{{ old('tags.0') }}"
                    class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition mb-2"
                    placeholder="Tag 1 (contoh: 450 VA)">
                <input type="text" name="tags[]" value="{{ old('tags.1') }}"
                    class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition mb-2"
                    placeholder="Tag 2 (contoh: 900 VA)">
                <input type="text" name="tags[]" value="{{ old('tags.2') }}"
                    class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition"
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-700">
                <a href="{{ route('admin.slo.kategori-layanan.index') }}" class="px-4 py-2 rounded-xl text-slate-300 hover:text-white hover:bg-slate-700 font-medium text-sm transition">
                    Batal
                </a>
